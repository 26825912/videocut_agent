...
from .script_breakdown import scipt_breakdown2excel,script_breakdown
from .asr_tools import AzureTranscriber
from .utils import download_video_from_url,lang_to_asr_lang,save_json_to_file
from .ai_copywrite import ai_copywrite
import json
...
def video_script_analysis_tool(video_url: str, language: str = "en-US") -> Dict[str, Any]:
    ...
    try:
        script_breakdown_data = script_breakdown(full_text)
        # relative_path = save_json_to_file(script_breakdown_data)
        return {
            "tool_return":{
                "relative_parsed_result": script_breakdown_data,
                "describe": "爆款视频拆解后的文案"
            },
        }
    except Exception as e:
