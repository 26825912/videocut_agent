...
def img_search_tool_v2(queries: List[str], 
    ...
    for query in queries:
        try:
            ...
            # 如果返回的是字典（包含 thumb 和 full），提取 full URL
            if isinstance(selected_img_url, dict):
                download_url = selected_img_url.get('full') or selected_img_url.get('thumb')
                if not download_url:
                    print(f"ERROR: 字典格式错误，缺少 URL 字段: {selected_img_url}")
                    continue
            else:
