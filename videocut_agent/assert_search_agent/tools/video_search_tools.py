...
def search_and_download(
    ...
    # 修复：处理字符串化的字典、真正的字典、以及普通字符串
    if isinstance(query, str) and query.strip().startswith('{'):
        # 字符串化的字典：'{"query": "cute kitten"}' 或 "{'query': 'cute kitten'}"
        try:
    ...
    if isinstance(category, dict):
        category = category.get('category', 'nature')
...
def video_serach_and_clip_tools(query,
    ...
    logger.info(f"处理后的参数 - query: {query}, category: {category}")
    ...
    # 检查是否搜索到视频
    if not videos_path_list:
        logger.error(f"未找到关键词 '{query}' 的视频素材")
        return {
            "tool_return": [{
                "error": f"未找到关键词 '{query}' 的视频素材，请尝试更换关键词",
                "query": query,
                "category": category
            }]
        }
