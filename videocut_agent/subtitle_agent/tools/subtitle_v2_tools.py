...
from tools.video_formats import VideoFormat
...
def audio2ass(audio_file,lang = "英文", save_file = None,need_punc=False):
    """根据识别结果生成ASS字幕文件"""
    ...
    try:
        if temp_wav:
            ...
            if words_data:
                ...
                logger.warning(f"识别到的单词数据为空，未生成ASS字幕文件")
                return None
    except Exception as e:
...
def video2ass(video_file, save_file = None,need_punc=False):
    ...
    try:
        ...
        # relative_path = os.path.relpath(output_ass, data_dir)
        logger.info(f"字幕文件生成完成: {ass_file}")
        return ass_file
...
@tool
def audio2ass_tool(audio_file,lang = "英文",need_punc=False):
    """
    字幕生成工具，可以将音频转换为ASS字幕文件
    input:
        audio_file: 音频文件相对路径
        lang: 字幕语言，默认英文，可选值为（英文，中文），必须在可选值内
        need_punc: 是否需要添加标点符号
    return:
        relative_path: 字幕文件相对路径
    """
    logger.info(f"开始处理音频文件: {audio_file}")
    audio_file = os.path.join(get_data_dir(),audio_file)
    ass_file = audio2ass(audio_file,lang,None,need_punc)
    return {
        "tool_return":[{
            "ass_file": ass_file,
            "describe": "生成的字幕文件相对路径"
        }]
    }


@tool
def video2ass_tool(video_file,lang = "英文",need_punc=False):
    """
    将视频文件识别后转换为ASS字幕文件
    input:
        video_file: 视频文件路径，相对于data目录
        lang: 字幕语言，默认英文，可选值为（英文，中文），必须在可选值内
        need_punc: 是否需要添加标点符号
    return:
        relative_path: 字幕文件相对路径
    ...
    ass_file = video2ass(video_file, lang,None,need_punc)
    return {
        "tool_return":[{
            "ass_file": ass_file,
            "describe": "生成的字幕文件相对路径"
...
if __name__ == "__main__":
    ...
    audio_test_pathv=r'\result_video\clip_audio\Azure_decode_temp_eeff6a81-3942-4616-9699-b92e61e07be6_0_concate_20260114154824.wav'
    video_editor_data = audio2ass(video_path)
